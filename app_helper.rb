...
module AppHelper
  def confirm_class
    ...
    class_message = 'Please enter 1 or 2 to create (1 Teacher) or (2 Student)'
  ...
  def confirm_speciality(class_name)
  ...
  def confirm_permission(class_name)
    if class_name == 'Student'
      ...
      permission_message = 'Do the student have parent permissions [Y/N]?'
  ...
  def make_person_selection
    ...
    selection = 'nil'
    until answered
      puts 'Please select the person by correct selection number'
  ...
  def make_book_selection
    ...
    selection = 'nil'
    until answered
      puts 'Please select the book by correct selection number'
  ...
  def find_id
    ...
    puts "There are #{@rentals.length} current rented books"
    until answered
      puts 'Please Select a person using the correct selection number for the person on the left'
